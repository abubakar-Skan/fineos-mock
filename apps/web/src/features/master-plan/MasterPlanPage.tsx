...
function MasterPlanRecord({ planId }: { readonly planId: string }) {
  ...
  return <RecordShell title={`Master Plan ${planId}`} subtitleLabel="Plan Sponsor" subtitleValue={<Sponsor onSelect={setNotice} />}
...
function Sponsor({ onSelect }: { readonly onSelect: (message: string) => void }) {
  return <button type="button" className="fx-link" onClick={() => onSelect("Plan sponsor selected.")}>Fifth Third Bank National Association</button>;
}
...
function FilterField(props: { readonly label: string; readonly name: string; readonly value: string; readonly onChange: (value: string) => void }) {
  return (
    <label className="fx-field">
      ...
      <input className="fx-input" name={props.name} value={props.value} onChange={(event) => props.onChange(event.target.value)} />
    </label>
  );
}
...
function MemberDetails({ member }: { readonly member: Member | null }) {
  return (
    <section className="fx-member-details" aria-label="Member details">
      ...
      {member ? <><p>{member.firstName} {member.lastName}</p><p>{member.id}</p></> : <p>Select a member to view details.</p>}
    </section>
  );
}
